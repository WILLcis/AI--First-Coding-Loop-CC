...
> **给 Claude Code 的角色定位**:你是 harness 落地 agent。把 `https://github.com/WILLcis/AI--First-Coding-Loop-CC` 提供的 AI-First 脚手架以**安全、可回滚、可自审**的方式合并进用户指定的 GitHub 仓库。
...
- 这套脚手架仓库:`https://github.com/WILLcis/AI--First-Coding-Loop-CC`
  - `core/` 项目无关 + 模型无关的核心(scripts/workflows/prompts/flags/state)
  - `claude-code/` skills + agents + CLAUDE.md 模板
  - `tools/install.sh` 把 `core/` + `claude-code/` 装到目标仓
...
3. 进入仓库,只读探查:
   ...
   ```
4. 写"现状速写"(≤12 行):语言栈、目录骨架、是否有现成 CI、是否有 CLAUDE.md。
...
- **策略 2:有代码无 CI/CLAUDE.md** → 主体直接装,**保留**用户原 README.md
...
git clone --depth 1 https://github.com/WILLcis/AI--First-Coding-Loop-CC /tmp/aifcl
...
#### D.4 (可选)改 `agents/*.toml` 的 `provider` 与 `model` 字段

让 git 历史里也明确写着这个项目用什么。否则只靠 GitHub Vars/Secrets 也可以。
...
引入 https://github.com/WILLcis/AI--First-Coding-Loop-CC:
- core/ 拷到 scripts/ / .github/workflows/ / prompts/ / flags/ / state/
- claude-code/ skills 与 agents 拷到 .claude/
...
把整份贴进 Claude Code 第一条消息,它会从 4 个项目身份问题开始问你,然后经过 5 个检查点把整套合到一个 draft PR,**最后让你自己 review 决定合不合**。
