...
> 适用:从空仓起步 · GitHub + Claude + Statsig + AWS + Linear · 创始人/CTO 单人推动
...
- [ ] **Anthropic Console 账号**:[https://console.anthropic.com](https://console.anthropic.com) → Settings → API Keys 建一个 key 待用
...
> 预算预估(月费,完全可控):Anthropic ~$50~$200(看 PR 数量)/ AWS dev+prod ~$80~$300 / Statsig 个人免费档 / Linear $8/人 / Sentry 个人免费档 / GitHub Team $4/人。**Week 0 总开销通常 < $300**。
...
- [ ] 本地解压你已有的 `creao-ai-first-harness.zip`,重命名根目录为你的项目名
- [ ] 进入项目目录,初始化 git:
  ```bash
  cd your-project
  git init -b main
...
**验收**:仓库上能看到 `.github/workflows/`、`scripts/`、`prompts/`、`flags/`、`CLAUDE.md`,且 push 后 GitHub Actions 标签页里 CI 已开始跑(此时大概率红——因为还没装依赖逻辑,不要慌)。

### 下午(约 2 小时)

- [ ] **填好 `CLAUDE.md`**:把 `[占位]` 都替换成你的产品描述、目录约定、本地起栈命令。**这一步是 ROI 最高的**——后面所有 agent 都靠它工作。
...
**目标**:Anthropic、Statsig、Linear、Sentry、Teams 五个外部服务全部接进 GitHub Secrets/Vars,工作流第一次能跨服务调通。

按顺序做(每个 ~30 分钟):

### 2.1 Anthropic + 三趟 AI 评审

- [ ] 仓库 → **Settings → Secrets and variables → Actions → New repository secret**
  - `ANTHROPIC_API_KEY` = Anthropic Console 里的 key
- [ ] 开一个测试 PR,确认 `.github/workflows/claude-review.yml` 的三个 job(quality / security / dependency)被触发并完成
...
- [ ] **agent 实现**:在 PR 描述里贴架构师的任务模板,触发实现(可用 `@claude` mention,或开一个新分支让 Claude Code 来跑)
...
7. `CLAUDE.md` 是真实的、不再有 `[占位]`,新 agent 只读它就能开始干活
...
8. **整周做完发现 agent 写不动东西** → CLAUDE.md 太空。回去把"如何起栈/如何测试/编码规范/禁区"写细。
