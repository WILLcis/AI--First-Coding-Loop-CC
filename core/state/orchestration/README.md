...
> 主 session 崩溃可断点续;事后可回看每一轮的 fan-out/fan-in 决策。
...
- **append-only 精神**:轮一旦开始写,**不要回头改前面轮的文件**(改了就破坏可审计性)
...
- ❌ 把这个目录 .gitignore 掉 —— 它是审计/comprehension 的基础
- ❌ 把 result.md 放敏感数据 —— sub-agent 返回应该是结构化 JSON 摘要,不是原始日志
- ❌ 删旧任务目录 —— 用 rotate_state(若你团队人多)或干脆留着,空间不值钱
